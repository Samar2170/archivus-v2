...
<div class="min-h-screen bg-gray-50">
	...
	<main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
				...
				<div class="relative">
					...
					<input
						...
						class="pl-9 pr-3 py-2 text-sm rounded-lg border border-gray-300
							focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 w-60"
					/>
				</div>
				<button
					type="submit"
		...
		{:else}
							...
							<tr class="hover:bg-gray-50 transition-colors">
								<td class="px-4 py-3">
									<div class="flex items-center gap-2">
										{#if file.IsImage}
											<Image class="h-4 w-4 text-pink-400 flex-shrink-0" />
										{:else}
											<File class="h-4 w-4 text-gray-400 flex-shrink-0" />
										{/if}
										<span class="text-sm font-medium text-gray-800 truncate max-w-xs" title={file.Name}>
											{file.Name}
										</span>
									</div>
								</td>
			...
			<p class="mt-3 text-sm text-gray-400">{files.length} file{files.length !== 1 ? 's' : ''}</p>
		{/if}
	</main>
</div>
