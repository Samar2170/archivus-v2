<script lang="ts">
	...
	function openItem(file: FileMetaData) {
		if (file.IsDir) {
			goto(`/?folder=${encodeURIComponent(file.NavigationPath || file.Path)}`);
	...
	async function handleDropOnFolder(targetFile: FileMetaData) {
		...
		const draggedFile = files.find((f) => f.id === draggedFileId || f.ID === draggedFileId);
		if (!draggedFile || draggedFile.ID === targetFile.ID) return;

		try {
			await moveFile(draggedFile.Path, targetFile.NavigationPath || targetFile.Path);
			await loadFiles();
		} catch (err) {
	...
	<main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
		...
		{:else if files.length === 0}
			<div class="flex flex-col items-center justify-center py-24 text-gray-400">
				...
				<p class="text-sm">Upload files or create a folder to get started.</p>
			</div>
		{:else}
			<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
				{#each files as file (file.ID || file.id)}
					...
					<div
						...
						on:dragover|preventDefault={() => { if (file.IsDir) dragOverFolder = file.ID || file.id; }}
						on:dragleave={() => { if (dragOverFolder === (file.ID || file.id)) dragOverFolder = null; }}
						on:drop|preventDefault={() => handleDropOnFolder(file)}
						on:click={() => openItem(file)}
						on:keydown={(e) => e.key === 'Enter' && openItem(file)}
						class="outline-none
							{dragOverFolder === (file.ID || file.id) ? 'ring-2 ring-indigo-400 rounded-xl' : ''}"
					>
						<FileCard {file} dragging={draggedFileId === (file.ID || file.id)} />
					</div>
				{/each}
			</div>
		{/if}
