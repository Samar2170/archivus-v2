<script lang="ts">
	...
	import { CheckSquare, Square, AlertCircle, Minus, ArrowUp } from 'lucide-svelte';
	...
	<div class="overflow-x-auto rounded-xl border border-gray-200">
		<table class="min-w-full divide-y divide-gray-200">
			<thead class="bg-gray-50">
				<tr>
					<th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 w-10"></th>
				...
				{#each todos as todo (todo.id)}
					<tr class="hover:bg-gray-50 transition-colors">
						<td class="px-4 py-3">
							...
							>
								{#if todo.status === 2}
									<CheckSquare class="h-5 w-5 text-indigo-500" />
								{:else}
									<Square class="h-5 w-5" />
								{/if}
							</button>
						</td>
						<td class="px-4 py-3 text-sm font-medium text-gray-800 {todo.status === 2 ? 'line-through text-gray-400' : ''}">
							{todo.title}
						</td>
						<td class="px-4 py-3">
							{#if todo.priority !== undefined}
								{@const p = PRIORITY_LABELS[todo.priority] ?? { label: 'Unknown', color: 'text-gray-500 bg-gray-50' }}
								<span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {p.color}">
									{p.label}
								</span>
							{/if}
						</td>
