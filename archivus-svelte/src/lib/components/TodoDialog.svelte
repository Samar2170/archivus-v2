...
<div class="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3">
	{#if fabOpen}
		<button
			on:click={() => { projectOpen = true; fabOpen = false; }}
		...
		<button
			on:click={() => { todoOpen = true; fabOpen = false; }}
			class="flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-medium text-gray-700
				shadow-lg ring-1 ring-gray-200 hover:bg-gray-50 transition-colors"
		>
			New todo
...
{#if fabOpen}
	<button class="fixed inset-0 z-30" aria-label="Close" on:click={() => (fabOpen = false)}></button>
{/if}
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
		<div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
			<div class="mb-4 flex items-center justify-between">
				...
				<button on:click={() => (projectOpen = false)} class="text-gray-400 hover:text-gray-600">
					<X class="h-5 w-5" />
				</button>
			</div>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
		<div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
			<div class="mb-4 flex items-center justify-between">
				...
				<button on:click={() => (todoOpen = false)} class="text-gray-400 hover:text-gray-600">
					<X class="h-5 w-5" />
				</button>
			</div>
			...
			<div class="space-y-3 mb-4">
				...
				<select
					...
					class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm
						focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-white"
				>
